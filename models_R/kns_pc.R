rank_weighted_factor_ws <- function(chars, features) {
  factor_ws <- copy(chars)
  factor_ws[, (features) := lapply(.SD, function(r) (r - mean(r)) / sum(abs(r - mean(r)))), .SDcols=features, by = eom]
  factor_ws[, (features) := lapply(.SD, function(w) ifelse(is.na(w), 0, w)), .SDcols=features]
...
kns <- function(d, rw_factor_d_raw, rw_factor_m, mkt_ret_daily, mkt_ret, features, opt_lambdas, use_pc) {
    ...
    names(dt) <- c("eom_ret", "ret_exc", "name", features, "me")
    dt
}


main <- function(chars, features, daily_ret) {
  rw_factors <- create_rw_factors(chars, daily_ret, features)
  mkt_rets <- create_mkt_rets(chars, daily_ret)

  pf_dates <- unique(subset(chars, ctff_test == TRUE)$eom_ret)
  ...
  test_chars <- chars |> filter(ctff_test == TRUE)
  test_chars[, (features) := lapply(.SD, function(r) (r - mean(r)) / sum(abs(r - mean(r)))), .SDcols=features, by = eom]
  test_chars[, (features) := lapply(.SD, function(w) ifelse(is.na(w), 0, w)), .SDcols=features]
  test_chars[, me := me / sum(me), by = eom]
  test_chars <- test_chars |> select(id, eom, me, all_of(features))

  opt_chars <- unique(test_chars$eom) |>
    map(.progress = TRUE, function(d) {
      test_chars_d <- test_chars |> filter(eom_ret)
      w_kns_pc_d <- w_kns_pc |> filter(eom_ret)
  ...
  opt_chars$w <- opt_chars |> select(all_of(features), "me") |> rowSums()
