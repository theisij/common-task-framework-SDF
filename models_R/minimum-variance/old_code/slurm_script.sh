#!/bin/bash
#SBATCH -J covariance                           # Job name
#SBATCH --array=1-3                       # Number of tasks (4 tasks for 4 outcomes)
#SBATCH -o covariance-estimation/slurm_output/output_%A_%a.txt                # Output file (%A=job ID, %a=array task ID)
#SBATCH -e covariance-estimation/slurm_output/error_%A_%a.txt                 # Error file 
...
OUTCOME=$(sed -n "${SLURM_ARRAY_TASK_ID}p" 'covariance-estimation/joblist.txt')
...
Rscript "covariance-estimation/submit_to_slurm.R" --outcome ${OUTCOME}